 ...
 */
void momScaledAddFlocr(FLOCR *lr, Real vr, FLOCR *la, Real va) {
    Real f, s;
    assert(vr > 0.0 && va > 0);
    f = va/vr;
    s = f;
 ...
 */
void momRescaleFlocr(FLOCR *lr, Real vnew, Real vold) {
    Real f, s;
...
void momShiftFmomr(FMOMR *m, Real u, Real x, Real y,
                   Real z) {
    FMOMR f;
    Real t, tx, ty, tz, txx, tyy, txy, tyz, txz, iu;
    const Real twosevenths = 2.0f / 7.0f;
 ...
 */
double momShiftFlocr(FLOCR *l, Real v, Real x, Real y,
                     Real z) {
    const Real onethird = 1.0f / 3.0f;
    Real iv, hx, hy, hz, tx, ty, tz;
    Real L, Lx, Ly, Lz, Lxx, Lxy, Lxz, Lyy, Lyz, Lxxx, Lxxy, Lxxz, Lxyy,
        Lxyz, Lyyy, Lyyz;
    Real Lxxxx, Lxxxy, Lxxyy, Lxyyy, Lyyyy, Lxxxz, Lxxyz, Lxyyz, Lyyyz;
...
void momEvalFmomrcm(const FMOMR *m, Real u, Real dir, Real x,
    ...
    Real xxx, xxy, xxz, xyy, yyy, yyz, xyz;
    Real tx, ty, tz, g0, g2, g3, g4;
...
double momFlocrAddFmomr5cm(FLOCR *l, Real v, FMOMR *m, Real u,
    ...
    Real u2, u3, u4;
    Real xx, xy, xz, yy, yz, zz;
    Real xxx, xxy, xyy, yyy, xxz, xyz, yyz;
    Real R2xx, R2xy, R2xz, R2yy, R2yz, R2x, R2y, R2z, R2, R3xx, R3xy, R3yy,
        R3xz, R3yz, R3x, R3y, R3z, R3, R4x, R4y, R4z, R4;
    Real T0, txx, tyy, t1, t1x, t1y, t1z, t1xx, t1yy, t2x, t2y, t2z, t2xx,
        t2yy, txxxx, tyyyy;
...
** Op Count = (*,+,-) = (,,) = 
*/
double momFlocrAddMono5(FLOCR *l, Real v, Real m, Real dir,
                        Real x, Real y, Real z, Real *tax,
                        Real *tay, Real *taz) {
    Real xx, xy, xz, yy, yz;
    Real T0, txx, tyy, t1, t1xx, t1yy, t2, txxxx, tyyyy;
...
void momEvalFlocr(FLOCR *l, Real v, Real x, Real y, Real z,
                  Real *fPot, Real *ax, Real *ay,
                  Real *az) {
    const Real onethird = 1.0f / 3.0f;
    Real xx, xy, xz, yy, yz, zz, xxx, xxz, yyy, yyz, xxy, xyy, xyz, xxxx,
        xxxy, xxyy, xyyy, yyyy, xxxz, xxyz, xyyz, yyyz;
    Real g1, A, Ax, Ay, Az, B, Bx, By, Bz, C, Cx, Cy, Cz, D, Dx, Dy, Dz;
    Real iv;
