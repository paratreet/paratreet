...
PARATREET_REGISTER_PER_LEAF_FN(DensityFn, CentroidData, (
  [](SpatialNode<CentroidData>& leaf, Partition<CentroidData>* partition) {
    ...
    for (int pi = 0; pi < leaf.n_particles; pi++) {
      auto& part = leaf.particle(pi);
      auto& Q = part.neighbors;
      auto rsq = Q[0].fKey;
      auto fBall = std::sqrt(rsq);
...
PARATREET_REGISTER_PER_LEAF_FN(ForceFn, CentroidData, (
  [](SpatialNode<CentroidData>& leaf, Partition<CentroidData>* partition) {
    auto oem = partition->opposite_effects_manager;

    for (int pi = 0; pi < leaf.n_particles; pi++) {
      auto& part = leaf.particle(pi);
      auto& Q = part.neighbors; // this might have k+1 or even k+2
      auto rsq = part.sphBallSq, fBall = std::sqrt(rsq);
  ...
  void ExMain::traversalFn(const BoundingBox& universe, ProxyPack<CentroidData>& proxy_pack, int iter) {
    ...
    start_time = CkWallTimer();
    proxy_pack.opposite_effects_manager.applyAccumulatedOpposingEffects(proxy_pack.partition);
    CkWaitQD();
