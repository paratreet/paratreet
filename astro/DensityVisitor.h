#ifndef ASTRO_DENSITYVISITOR_H_
#define ASTRO_DENSITYVISITOR_H_
...
public:
  static bool open(const SpatialNode<CentroidData>& source, SpatialNode<CentroidData>& target) {
    ...
    for (int i = 0; i < target.n_particles; i++) {
      if (target.particle(i).neighbors.size() < k) return true;
      if(Space::intersect(source.data.box, target.particle(i).position, target.particle(i).neighbors[0].fKey))
        return true;
    }
    return false;
  }
  ...
  static void leaf(const SpatialNode<CentroidData>& source, SpatialNode<CentroidData>& target) {
    for (int i = 0; i < target.n_particles; i++) {
      auto& Q = target.particle(i).neighbors;
