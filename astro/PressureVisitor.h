#ifndef ASTRO_PRESSUREVISITOR_H_
#define ASTRO_PRESSUREVISITOR_H_
...
public:
  bool open(const SpatialNode<CentroidData>& source, SpatialNode<CentroidData>& target) {
    ...
    for (int i = 0; i < target.n_particles; i++) {
      if(Space::intersect(source.data.box, target.particle(i).position, target.particle(i).sphBallSq))
        return true;
    }
    return false;
  }
  ...
  void leaf(const SpatialNode<CentroidData>& source, SpatialNode<CentroidData>& target) {
    for (int i = 0; i < target.n_particles; i++) {
      Particle& a = target.particle(i);
      auto rsq = a.sphBallSq;
      for (int j = 0; j < source.n_particles; j++) {
        const Particle& b = source.particle(j);
...
#endif // ASTRO_PRESSUREVISITOR_H_
