#ifndef ASTRO_COLLISIONVISITOR_H_
#define ASTRO_COLLISIONVISITOR_H_
...
public:
  ...
  static Real getCollideTime(const Particle& a, const Particle& b) {
    ...
    auto vRel = a.velocity - b.velocity + (astroConf.max_timestep / 2.) * (a.acceleration - b.acceleration); // this is kinda wrong cause accelerations are not updated properly
  ...
  void leaf(const SpatialNode<CentroidData>& source, SpatialNode<CentroidData>& target) {
    for (int i = 0; i < target.n_particles; i++) {
      for (int j = 0; j < source.n_particles; j++) {
        auto& sp = source.particle(j);
        auto& tp = target.particle(i);
...
#endif // ASTRO_COLLISIONVISITOR_H_
