  ...
  [](SpatialNode<CentroidData>& leaf, Partition<CentroidData>* partition)
{
	...
	L = astroConf.fPeriod.x;
	fEwCut2 = fEwCut*fEwCut*L*L;
	fInner2 = 1.2e-3*L*L;
	nEwReps = nEwReps > astroConf.nReplicas ? nEwReps : astroConf.nReplicas;
	alpha = 2.0/L;
	alpha2 = alpha*alpha;
	...
	for(j = 0; j < leaf.n_particles; j++) {
            ...
            for (ix=-nEwReps;ix<=nEwReps;++ix) {
                bool bInHolex = (ix >= -astroConf.nReplicas && ix <= astroConf.nReplicas);
                x = dx.x + ix*L;
                for(iy=-nEwReps;iy<=nEwReps;++iy) {
                    bool bInHolexy = (bInHolex && iy >= -astroConf.nReplicas && iy <= astroConf.nReplicas);
                    y = dx.y + iy*L;
                    for(iz=-nEwReps;iz<=nEwReps;++iz) {
                        bool bInHole = (bInHolexy && iz >= -astroConf.nReplicas && iz <= astroConf.nReplicas);
                        /*
          ...
          leaf.applyEffect(j, {acc, (Real)fPot, 0.});
        }
	return;
 }));
